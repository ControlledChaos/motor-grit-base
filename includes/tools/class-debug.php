...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'mg_check_wp_debug_define' );
}

function mg_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'mg_site_development' );
		if ( $dev_mode ) {
			mg_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			mg_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function mg_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = mg_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'mg_set_wp_debug_define', $turn_it_on );
 ...
 */
function mg_find_wpconfig_path() {
...
function mg_deactivation() {
    ...
    set_mg_wp_debug_off( false );
}

function set_mg_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = mg_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'mg_set_wp_debug_define', $turn_it_off );
