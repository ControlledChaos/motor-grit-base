...
$tabs = [
    ...
    sprintf(
        ...
        esc_html__( 'Dashboard', 'motor-grit' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Menu', 'motor-grit' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Admin Pages', 'motor-grit' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Site Users', 'motor-grit' )
    ),
    ...
    sprintf(
        ...
        esc_html__( 'Meta/SEO', 'motor-grit' )
    )

];

// Apply a filter to the tabs array for adding tabs.
$page_tabs = apply_filters( 'mg_tabs_site_settings', $tabs );
...
if ( 'dashboard' == $active_tab  ) {
    $section = 'mg-site-dashboard';
    $fields  = 'mg-site-dashboard';
} elseif ( 'admin-menu' == $active_tab ) {
    $section = 'mg-site-admin-menu';
    $fields  = 'mg-site-admin-menu';
} elseif ( 'admin-pages' == $active_tab ) {
    $section = 'mg-site-admin-pages';
    $fields  = 'mg-site-admin-pages';
} elseif ( 'users' == $active_tab ) {
    $section = 'mg-site-users';
    $fields  = 'mg-site-users';
} elseif ( 'meta-seo' == $active_tab ) {
    $section = 'mg-site-meta-seo';
    $fields  = 'mg-site-meta-seo';
} else {
    $section = null;
    $fields  = null;
}
...
$do_section = apply_filters( 'mg_section_site_settings', $section );
$do_fields  = apply_filters( 'mg_fields_site_settings', $fields );
 ...
 */
if ( 'dashboard' == $active_tab  ) {
    $save = __( 'Save Dashboard', 'motor-grit' );
} elseif ( 'admin-menu' == $active_tab ) {
    $save = __( 'Save Menu', 'motor-grit' );
} elseif ( 'admin-pages' == $active_tab ) {
    $save = __( 'Save Pages', 'motor-grit' );
} elseif ( 'users' == $active_tab ) {
    $save = __( 'Save Users', 'motor-grit' );
} elseif ( 'meta-seo' == $active_tab ) {
    $save = __( 'Save Meta', 'motor-grit' );
} else {
    $save = __( 'Save Settings', 'motor-grit' );
}

// Apply a filter for new tabs added by another plugin or from a theme.
$button = apply_filters( 'mg_save_site_settings', $save );
...
<div class="wrap">
	<?php echo sprintf(
        ...
        get_bloginfo( 'name' ),
        esc_html__( 'Settings', 'motor-grit' )
    ); ?>
    <p class="description"><?php esc_html_e( 'Customize the way WordPress is used.', 'motor-grit' ); ?></p>
