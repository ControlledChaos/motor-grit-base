...
class Admin_Pages {
    ...
    public function about_plugin() {
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_label = get_field( 'mg_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'mg_site_plugin_link_label' ) );
        }
        ...
        // Otherwise use Site Plugin as the label.
            ...
            $label = __( 'Site Plugin', 'motor-grit' );
        }
        ...
        if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'mg_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'mg_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'motor-grit' );
        }
    ...
    public function about_plugin_output() {
        ...
        require MG_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'motor-grit' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'motor-grit' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once MG_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once MG_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'motor-grit' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'motor-grit' ),
            'Motor & Grit'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'motor-grit' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'motor-grit' )
         );
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'motor-grit' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'motor-grit' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'motor-grit' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'mg_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'mg_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'motor-grit' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'mg_image_column_head', $name );
