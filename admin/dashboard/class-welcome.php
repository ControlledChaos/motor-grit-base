...
class Welcome {
    ...
    public function __construct() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			$dismiss = get_field( 'mg_remove_welcome_dismiss', 'option' );

		// If ACF is not active, get the field from the WordPress options page.
		} else {
			$dismiss = get_option( 'mg_remove_welcome_dismiss' );
		}
		...
		// If ACF is active, get the field from the ACF options page.
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			$welcome = get_field( 'mg_custom_welcome', 'option' );
		} else {
	...
	public function widget_areas() {
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - First Area', 'motor-grit' ),
			'id'            => 'mg_welcome_widget_first',
			'description'   => __( '', 'motor-grit' ),
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Second Area', 'motor-grit' ),
			'id'            => 'mg_welcome_widget_second',
			'description'   => __( '', 'motor-grit' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
		...
		register_sidebar( [
			'name'          => __( 'Welcome Panel - Third Area', 'motor-grit' ),
			'id'            => 'mg_welcome_widget_last',
			'description'   => __( '', 'motor-grit' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3>',
	...
	public function welcome_panel() {
		...
		include_once MG_PATH . 'admin/dashboard/partials/welcome-panel.php';
	...
	public function styles() {
        ...
        if ( $screen->id == 'dashboard' ) {
            wp_enqueue_style( MG_ADMIN_SLUG . '-welcome', MG_URL .  'assets/css/welcome.min.css', [], null, 'screen' );
        }
