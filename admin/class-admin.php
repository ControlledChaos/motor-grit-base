...
class Admin {
	...
	private function dependencies() {
		...
		require_once MG_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'mg_acf_activate_settings_page' ) ) {
			include_once MG_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( class_exists( 'acf_pro' ) && ! get_option( 'mg_acf_activate_settings_page' ) ) {
			include_once MG_PATH . 'admin/class-settings-fields-site-acf.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once MG_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once MG_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once MG_PATH . 'admin/class-admin-toolbar-menus.php';

		// Functions for various admin pages and edit screens.
		require_once MG_PATH . 'admin/class-admin-pages.php';

		// Import custom fields for editing, if ACF Pro is active.
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			include_once MG_PATH . 'admin/class-fields-import.php';
		}

		// Filter by page template.
		require_once MG_PATH . 'admin/class-admin-template-filter.php';
	...
	public function admin_footer() {
		...
		if ( class_exists( 'acf_pro' ) || ( class_exists( 'acf' ) && class_exists( 'acf_options_page' ) ) ) {
			 ...
			 */
			$credit = get_field( 'mg_admin_footer_credit', 'option' );
			$link   = get_field( 'mg_admin_footer_link', 'option' );
			...
			if ( $credit && $link ) {
				$footer = sprintf(
					...
					$site,
					esc_html__( 'website designed & developed by', 'motor-grit' ),
					esc_url( $link ),
					$credit
				);
			...
			// If no input we use the name of the site.
				...
				$footer = sprintf(
					'%1s %2s. ',
					$site,
					esc_html__( 'website powered by WordPress', 'motor-grit' )
				);
			}
		 ...
		 */
			...
			$credit = sanitize_text_field( get_option( 'mg_footer_credit' ) );
			$link   = esc_url_raw( get_option( 'mg_footer_link' ) );
		...
		$admin_footer = apply_filters( 'mg_admin_footer', $footer );
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( MG_ADMIN_SLUG . '-admin', MG_URL . 'admin/assets/css/admin.min.css', [], MG_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( MG_ADMIN_SLUG . '-tooltips', MG_URL . 'admin/assets/css/tooltips.min.css', [], MG_VERSION, 'all' );
		 ...
		 */
		if ( class_exists( 'acf' ) ) {
			wp_enqueue_style( MG_ADMIN_SLUG . '-acf', MG_URL . 'admin/assets/css/acf.css', [], MG_VERSION, 'all' );
		}
		 ...
		 * @since 1.0.0
		 */
		$welcome = get_option( 'mg_custom_welcome' );
		if ( $welcome ) {
			wp_enqueue_style( MG_ADMIN_SLUG . '-welcome', MG_URL . 'admin/assets/css/welcome.css', [], MG_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require MG_PATH . 'partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( MG_ADMIN_SLUG . '-conditionalize', MG_URL . 'admin/assets/js/admin.js', [ 'jquery' ], MG_VERSION, true );

		// Enqueue scripts for backend functionality of this plugin.
		wp_enqueue_script( MG_ADMIN_SLUG . '-admin', MG_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], MG_VERSION, true );
